...
#' generate_type3_data("meta_data.csv", "gene_count.csv", selected_clusters, "type_three")
#' }
#'
generate_type3_data <- function(meta_data_file, gene_count_file, selected_clusters, type) {
  ...
  get_gradation_2d_celltype4 <- rbind(array_2, array_3)
  ...
  new_data <- spatial_spot_nUMI(result_matrices, selected_clusters, org_data, spot_nUMI_filename = paste0(type, "_spatial_spot_nUMI.csv"), new_data_filename = paste0(type, "_new_data.csv"))
